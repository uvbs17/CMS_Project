...
function getNextTransactionID()
{
	...
	// Check if there is any existing transaction ID
	$query = "SELECT transaction_id FROM student_fee ORDER BY transaction_id DESC LIMIT 1";
	$result = mysqli_query($con, $query);

	if (mysqli_num_rows($result) > 0) {
		// If there is an existing transaction ID, increment the last 5 digits by 1
		$row = mysqli_fetch_assoc($result);
		$last_transaction_id = $row['transaction_id'];
		mysqli_free_result($result);

		$last_digits = substr($last_transaction_id, -5);
		$next_digits = str_pad(intval($last_digits) + 1, 5, '0', STR_PAD_LEFT);
		$new_transaction_id = "BMU-" . $year . $next_digits;
	} else {
		// If there is no existing transaction ID, start from 00001
		$new_transaction_id = "BMU-" . $year . "00001";
	}
...
if (isset($_GET['enrollment_no'])) {
	...
	if ($result) {
		...
		$query2 = "INSERT INTO `student_fee`(`fee_voucher`,`transaction_id`, `enrollment_no`,`paid_sem`, `amount`, `posting_date`, `status`) 
				  VALUES (NULL,'$transaction_id','$enro','$sem','$amount',CURRENT_TIMESTAMP,'paid')";

		if (mysqli_multi_query($con, "$query1;$query2")) {
...
<body>
	...
	<main role="main" class="col-xl-10 col-lg-12 col-md-12 ml-sm-auto px-md-4 mb-2 w-100">
		<div class="sub-main">
			...
			<div class="row">
				<div class="col-md-12">
					<form id="filter" action="Student-fee.php" method="post">
						<div class="row mt-3">
							<div class="col-md-6">
					...
					<script>
						const select = document.getElementById("fees_stat_select");
					...
					<section class="mt-3">
						<table style="margin-left: 0 ; margin-right:0;"
							...
							if ($status == 'paid') {
								...
								<?php
								$query = "SELECT * FROM student_fee AS f INNER JOIN student_info AS s ON f.enrollment_no = s.enrollment_no WHERE s.dept_id=$admin_dept_id LIMIT " . ($page - 1) * $rows_per_page . ",$rows_per_page";
								$run = mysqli_query($con, $query);
								if ($run) {
									while ($row = mysqli_fetch_array($run)) {
										echo "<tr>";
										echo "<td>" . $sr++ . "</td>"; ?>
										<td><a target="_blank" title="Generate Invoice"
										...
										<td width='175'>
											<a style=" border-radius: 20px;" class="btn btn-danger"
												href="Student-fee.php?fee_voucher=<?= $row['fee_voucher'] ?>">Delete</a>
											<?php echo "</tr>";
									}
								} else {
								...
								$total_rows = mysqli_num_rows(mysqli_query($con, "SELECT * FROM student_info WHERE dept_id=$admin_dept_id  AND fees_status='paid'"));
								...
								<?php } else { ?>
									...
									<?php
									$query = "SELECT * FROM student_info AS s INNER JOIN courses AS c ON s.course_code = c.course_code WHERE s.dept_id=$admin_dept_id AND s.fees_status='unpaid' LIMIT " . ($page - 1) * $rows_per_page . ",$rows_per_page";
									$run = mysqli_query($con, $query);
									while ($row = mysqli_fetch_array($run)) {
										echo "<tr>";
										echo "<td>" . $sr++ . "</td>";
									...
									$total_rows = mysqli_num_rows(mysqli_query($con, "SELECT * FROM student_info WHERE dept_id=$admin_dept_id AND fees_status='unpaid'"));
									$total_pages = ceil($total_rows / $rows_per_page);
