...
if (isset($_POST['sub'])) {
	// Get form data
	$action = $_POST['action'];
	$deptartment_id = mysqli_real_escape_string($con, $_POST['dept_id']);
	$department_name = mysqli_real_escape_string($con, $_POST['dept_name']);
	
	//Perform insert or update based on action
	if ($action == 'insert') {
		$query = "INSERT INTO department (id,dept_id,dept_name) 
                VALUES (NULL, '$deptartment_id','$department_name');";
		...
		$query = "UPDATE department SET dept_name='$department_name' WHERE dept_id='$deptartment_id'";
		$run = mysqli_query($con, $query);
		if ($run) {
...
if (isset($_GET['del_dept_id'])) {
	...
	$query2 = "DELETE FROM department where dept_id='$del_dept_id'";
		...
		<div class="sub-main">
			...
			<div class="container">
				<div class="modal-body">
					...
					function get_course_data($con, $dept_id)
					{
					...
					if (isset($_GET['dept_id'])):
						...
						$row = get_course_data($con, $dept_id);
						?>
							...
							<div class="row mt-3">
								<div class="col-md-6">
									<div class="form-group">
										...
										<input value="<?php echo !empty($row['dept_id']) ? $row['dept_id'] : ''; ?>"
											type="text" name="dept_id" class="form-control" required
											placeholder="Enter Department ID" readonly>
									</div>
								</div>
								<div class="col-md-6">
									<div class="form-group">
										<label for="exampleInputPassword1">Deaprtment Name:</label>
										<input value="<?php echo !empty($row['dept_name']) ? $row['dept_name'] : ''; ?>"
											type="text" name="dept_name" class="form-control" required
							...
							<div class="row mt-3">
								<div class="col-md-6">
									<div class="form-group">
										...
										<label for="exampleInputEmail1">Department ID: </label>
										<input value="" type="text" name="dept_id" class="form-control" required
			...
			<div class="col-md-10 ml-2 table-responsive-md">
				<section class="mt-3">
					<table style="margin-left: 0 ; margin-right:0;"
						...
						$query = "SELECT * FROM department LIMIT " . ($page - 1) * $rows_per_page . ",$rows_per_page";
						$run = mysqli_query($con, $query);
						while ($row = mysqli_fetch_array($run)) {
							...
							<td width='170' class="button">
								<a class="btn btn-primary mr-1"
									href="department.php?dept_id=<?= $row['dept_id'] ?>">Update</a>
								<a class="btn btn-danger" href="department.php?del_dept_id=<?= $row['dept_id'] ?>"
						...
						$total_rows = mysqli_num_rows(mysqli_query($con, "SELECT * FROM department"));
