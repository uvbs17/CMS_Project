...
if(isset($_GET['del_user_id']))
{
	...
	$pass = $_GET['password'];
	$del_query = "DELETE FROM `admin_info` WHERE user_id='$del_id' AND password='$pass'";
			...
			<div class="container">
				<div class="modal-body">
					...
					if (isset($_GET['user_id'])):
						...
						$query = "SELECT * FROM admin_info WHERE user_id = '$user_id' AND password = '$password'  LIMIT 1";
						$result = mysqli_query($con,$query);

						if(mysqli_num_rows($result) > 0)
						{
							$row = mysqli_fetch_array($result);
							$cdept_id = $row['dept_id'];
							$query2 = "SELECT * FROM department WHERE dept_id = '$cdept_id'";
								...
								<div class="col-md-6">
									<div class="form-group">
										...
										<input value="<?php echo !empty($row['user_id']) ? $row['user_id'] : ''; ?>"
											type="text" name="user_id" class="form-control" required
								...
								<div class="col-md-6">
									<div class="form-group">
										...
										<input value="<?php echo !empty($row['password']) ? $row['password'] : ''; ?>"
											type="text" name="password" class="form-control" required
							...
							<div class="row">
								<div class="col-md-6">
									<div class="form-group">
										...
										<select class="browser-default custom-select" name="dept_id" required="">
											<?php echo !empty($row['dept_id']) ? '<option value="' . $row['dept_id'] . ' " selected hidden>' . $row2['dept_name'] . '</option>' : ''; ?>

											<?php
											$query_dept = "SELECT * FROM department";
											$run = mysqli_query($con, $query_dept);
											while ($row_stream = mysqli_fetch_array($run)) {
												echo "<option value='" . $row_stream['dept_id'] . "'>" . $row_stream['dept_name'] . "</option>";
											}
											?>
										</select>
									</div>
							...
							<div class="row">
								<div class="col-md-6">
									<div class="form-group">
										<label for="exampleInputEmail1">Department ID:</label>
										<select class="browser-default custom-select" name="dept_id" required="">
											<option value="Select a department" selected>Select a Department</option>
			...
			<div class="col-md-10 ml-2 table-responsive-md">
				<section class="mt-3">
					<table style="margin-left: 0 ; margin-right:0;"
						...
						$query = "SELECT * FROM admin_info LIMIT " . ($page - 1) * $rows_per_page . ",$rows_per_page";
						$run = mysqli_query($con, $query);
						while ($row = mysqli_fetch_array($run)) {
							...
							<td width='170' class="button">
								<a class="btn btn-primary mr-1"
									href="admin.php?user_id=<?= $row['user_id'] ?>&password=<?= $row['password']?>">Update</a>
								<a class="btn btn-danger" href="admin.php?del_user_id=<?=$row['user_id']?>&password=<?=$row['password']?>"
						...
						$total_rows = mysqli_num_rows(mysqli_query($con, "SELECT * FROM admin_info"));
